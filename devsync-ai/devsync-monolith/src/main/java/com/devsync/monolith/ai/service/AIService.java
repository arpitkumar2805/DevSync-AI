...
public class AIService {
    ...
    public String chat(String userMessage, UUID projectId, UUID userId) {
        log.debug("Chat request for project {}", projectId);
        ...
        log.debug("Retrieved {} tasks for chat context", tasks.size());

        // Get user details
        String userContextStr = "Current User ID: " + userId;
        try {
            User currentUser = userRepository.findByIdAndDeletedFalse(userId).orElse(null);
            if (currentUser != null) {
    ...
    protected String callAiAndLog(String prompt, String feature, String entityType, UUID entityId, UUID userId) {
        log.info("Starting AI request for feature: {}, user: {}", feature, userId);

        if (aiApiKey == null || aiApiKey.trim().isEmpty() || aiApiKey.equals("dummy-key-value")) {
            log.error("AI provider API key is missing. Please configure NVIDIA_API_KEY.");
            throw new IllegalArgumentException("AI provider API key is missing. Please configure NVIDIA_API_KEY.");
        }
        ...
        try {
            ...
            // The configured NVIDIA NIM model is a reasoning model that otherwise echoes its
            // internal chain-of-thought into the answer; "/no_think" plus this instruction
            // keeps responses direct, matching what earlier (non-reasoning) providers returned.
            response = chatClient.prompt()
                    .system("/no_think Respond directly and concisely. Do not show your reasoning or chain of thought.")
                    .user(prompt)
                    .call()
                    .content();
            log.info("Received AI response for feature {} in {} ms", feature, System.currentTimeMillis() - startTime);
        } catch (Exception e) {
            log.error("Failed during LLM call for feature {}: {}", feature, e.getMessage(), e);
            throw e;
        }
        ...
        try {
            ...
            log.warn("Failed to save AI request log to database: {}", e.getMessage());
        }
